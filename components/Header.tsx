             ...
             {currentUser ? (
              <>
                ...
                <div className="hidden md:block">
                  <button onClick={onLogout} className="px-4 py-2 text-base font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 shadow-sm transition-colors">
                    Logout
                  </button>
                </div>
              </>
              ...
              <div className="hidden md:flex items-center gap-2">
                 <button onClick={() => navigateTo('login')} className="px-4 py-2 text-base font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 shadow-sm transition-colors">
                    Login / Sign Up
                </button>
              </div>
            )}
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 flex flex-col items-stretch">
            {currentUser ? (
              <>
                ...
                <button onClick={() => { onLogout(); setIsMenuOpen(false); }} className="w-full mt-4 text-left px-3 py-2 text-base font-medium text-white bg-red-600 rounded-lg hover:bg-red-700">
                  Logout
                </button>
              </>
            ) : (
                <>
                    <NavLink view="dashboard">Public Dashboard</NavLink>
                    <NavLink view="track">Track Issue</NavLink>
                    <button onClick={() => { navigateTo('login'); setIsMenuOpen(false); }} className="w-full mt-4 text-left px-3 py-2 text-base font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">
                        Login / Sign Up
                    </button>
                </>
            )}
