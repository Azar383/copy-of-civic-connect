...
import { summarizeResolvedIssues } from '../services/geminiService';
...
const PublicDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    summarizeResolvedIssues(resolved)
      .then(setSummary)
      .finally(() => setIsLoadingSummary(false));
  }, []);

  const SummaryDisplay: React.FC<{ text: string }> = ({ text }) => {
    const html = text
      .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
      .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer" class="text-blue-500 hover:underline">$1</a>')
      .replace(/\n/g, '<br />');
    return <div dangerouslySetInnerHTML={{ __html: html }} className="text-slate-600 dark:text-slate-300 leading-relaxed" />;
  };
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="bg-white dark:bg-slate-800/50 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-4 sm:p-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-4">Community Impact Summary</h3>
        {isLoadingSummary ? (
